...
//VuxInter/Demo/_5U_InsideIO.cpp/CInsideIO2/_03_UpdateUserNameOrPassword().h

//$$VUNIT(BEGIN,TESTFUNCPLUG/CInsideIO2::_03_UpdateUserNameOrPassword())
#ifdef _VUDEBUG
void CInsideIO2::_03_UpdateUserNameOrPassword_vpls_(){ VuxDbgBrkP()
{
    CString sql;
    ...
    GetDlgItemText(0x1001,  sUserName);
    GetDlgItemText(0x1002,  sPassword);
    GetDlgItemText(0x1003, sPassword2);
...
#else
void CInsideIO2::_03_UpdateUserNameOrPassword_vpls_(){__VUFNID__(0);VuxDbp(0,0,15);
{
...
VUXRLNFVB(2,2,2);    CString sUserName, sPassword, sPassword2; VUXLV("CString",sUserName,2); VUXLVS("CString",sPassword,3); VUXLVS("CString",sPassword2,4);
    ...
    /*[学习提示：局部输入应用]    以下三行代码，是从界面读取数据的语句，可以视为赋值语句。	选中一行代码，用局部输入(替换赋值语句)功能， 可以将原赋值语句	  改为普通赋值语句，并可在表格中设置变量的值。	*/
VUXRLNF(3,3);    GetDlgItemText(0x1001,  sUserName);
VUXRLNF(4,4);    GetDlgItemText(0x1002,  sPassword);
VUXRLNF(5,5);    GetDlgItemText(0x1003, sPassword2);
...
void CInsideIO2::_03_UpdateUserNameOrPassword_vpls()
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {CInsideIO2::_03_UpdateUserNameOrPassword(); return;}
...
//$$VUNIT(END,TESTFUNCPLUG/CInsideIO2::_03_UpdateUserNameOrPassword())
