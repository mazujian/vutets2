...
//VuxInter/Demo/_1U_Begin.c/_01_add2_d23b6c4e.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_01_add2(int,int))
#ifdef _VUDEBUG
static int _01_add2_vpls_(int a,int b){ VuxDbgBrkP()
{
...
#else
static int _01_add2_vpls_(int a,int b){__VUFNINFO__("_01_add2","int",0);VuxDbp(0,0,1);
{
    /*[学习提示：巧用示例，快速掌握单元测试]	示例集实际上是一部C/C++单元测试简明教程，可以浏览已测试的用例和结果，	  可以对照练习，可以体验边开发边测试。	示例代码中的学习提示，以及数据中的用例说明或数据后的注释，简明扼要地	  讲解了单元测试及VU4使用要点。	示例涵盖了C/C++单元测试的难点和解决方法，掌握了这些方法，并灵活运用，	  测试自己的代码就不会困难。	如果您只是学习单元测试，VU4的示例集是最好的教程，可以轻松掌握最先进的	  单元测试技术，掌握单元测试的要点、难点和解决问题的思路和方法。	如果您需要在自己的项目中评估VU，在申请试用License前，最好先掌握示例所	  阐述的要点和技巧，这样才能轻松地测试自己的代码，顺利完成评估。	*/
...
static int _01_add2_vpls(int a,int b)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _01_add2(a,b);}
	else if(VuxIsRepeatRun()) {return _01_add2_vpls_(a,b);}

#ifdef _VUDEBUG
    {int vuxRet =  VuxIsDebugPlug() ? _01_add2_vpls_(a,b) : _01_add2(a,b);
#else
    {int vuxRet = _01_add2_vpls_(a,b);
#endif
    TEST_ASSERT_RETURN("int", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_01_add2(int,int))
