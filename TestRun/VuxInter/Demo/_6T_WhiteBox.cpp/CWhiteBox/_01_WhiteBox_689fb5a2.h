...
//VuxInter/Demo/_6T_WhiteBox.cpp/CWhiteBox/_01_WhiteBox_689fb5a2.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/CWhiteBox::_01_WhiteBox(int,int,int))
#ifdef _VUDEBUG
int CWhiteBox::_01_WhiteBox_vpls_(int A,int B,int X){ VuxDbgBrkP()
{
...
#else
int CWhiteBox::_01_WhiteBox_vpls_(int A,int B,int X){__VUFNINFO__("_01_WhiteBox","int",0);VuxDbp(0,0,7);
{
    /*[学习提示：白盒覆盖]    意义：衡量测试完整度；找出遗漏用例。	常用覆盖：语句、条件值、判定、MC/DC、分支、路径，VU全部支持。	推荐的覆盖：MC/DC。MC/DC是航空标准，强度足够。MC/DC要求每个	              条件独立影响判定结果，实际上已包含语句、条件值、判定覆盖。				  分支包含空分支，空分支覆盖意义不大，非空分支的覆盖实际上就是判定覆盖；				  路径依赖于代码构成，当代码中含有多个逻辑块时，会组合出海量路径，				  因此，不必要求完成全部路径覆盖。	红绿灯机制：左边函数列表显示用绿、红、黄灯表示测试状况，	            红灯：有错误(失败的测试)，				黄灯：覆盖未达标，				黄红灯：有错误且覆盖未达标，				绿灯：无错误且覆盖达标。	覆盖标准：可在 工具->设定->统计与报告，设定覆盖标准，默认为：	          语句、条件值、判定、MC/DC达到100%。    */    
...
int CWhiteBox::_01_WhiteBox_vpls(int A,int B,int X)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return CWhiteBox::_01_WhiteBox(A,B,X);}
...
//$$VUNIT(END,TESTFUNCPLUG/CWhiteBox::_01_WhiteBox(int,int,int))
