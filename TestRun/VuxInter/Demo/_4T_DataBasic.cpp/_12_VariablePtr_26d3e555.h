...
//VuxInter/Demo/_4T_DataBasic.cpp/_12_VariablePtr_26d3e555.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_12_VariablePtr(int*,int*,int))
#ifdef _VUDEBUG
static int _12_VariablePtr_vpls_(int* item,int* items,int count){ VuxDbgBrkP()
{
...
#else
static int _12_VariablePtr_vpls_(int* item,int* items,int count){__VUFNINFO__("_12_VariablePtr","int",0);VuxDbp(0,0,13);
{
    /*[学习提示：变量指针赋值]    1) 适用场景: 适用于需比较指针本身，而不是比较指针所指对象的值的情形。    2) 给第一变量赋值: 在单元格编辑状态下，右键菜单选择“变量指针赋值”，    在弹出的窗口中设定变量名和类型，自动在单元格中填写$fv开头的一串字符，赋值即完成。    第一变量可以是数组，也可以是单个变量。    3) 给第二变量赋值: 第二变量通常指向第一变量或第一变量的一项，分三种情形：    a, 与第一变量无关，则同样调用“变量指针赋值”指令进行赋值；    b, 与第一变量相等，直接拷贝第一变量的值；    c, 指向第一变量的某项，填写第一变理的值+序号，如第一变量的值为$vpe08e0fea，       则第二变量填写$vpe08e0fea+1，将指向第1项(从0开始）。	*/
...
static int _12_VariablePtr_vpls(int* item,int* items,int count)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _12_VariablePtr(item,items,count);}
	else if(VuxIsRepeatRun()) {return _12_VariablePtr_vpls_(item,items,count);}

#ifdef _VUDEBUG
    {int vuxRet =  VuxIsDebugPlug() ? _12_VariablePtr_vpls_(item,items,count) : _12_VariablePtr(item,items,count);
#else
    {int vuxRet = _12_VariablePtr_vpls_(item,items,count);
#endif
    TEST_ASSERT_RETURN("int", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_12_VariablePtr(int*,int*,int))
