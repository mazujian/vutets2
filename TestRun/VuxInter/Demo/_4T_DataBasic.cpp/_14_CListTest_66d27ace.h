...
//VuxInter/Demo/_4T_DataBasic.cpp/_14_CListTest_66d27ace.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_14_CListTest(clist_t*))
#ifdef _VUDEBUG
static int _14_CListTest_vpls_(clist_t* pDatas){ VuxDbgBrkP()
{
...
#else
static int _14_CListTest_vpls_(clist_t* pDatas){VUX_VARIABLE(1)  __VUFNINFO__("_14_CListTest","int",0);VuxDbp(0,0,10);
{
     /*[学习提示：C语言链表的赋值]    1、如只需构建N个item，只需将最后一个item的pNext设为NULL    2、如需构建N个item，且需给item的成员赋值，则只需填值，最后item的pNext自动设为NULL    3、如需构建循环链表，可用调用脚本的ptrcpy，将最后item的pNext设为指向首个item    4、C语言双向链表，请参考_2T_SendMail.c的_08_send_mail函数，第一个参数是双向链表    5、C++双向链表，请参考_3T_MyClass.cpp的_02_Response_ok	*/   
...
static int _14_CListTest_vpls(clist_t* pDatas)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _14_CListTest(pDatas);}
	else if(VuxIsRepeatRun()) {return _14_CListTest_vpls_(pDatas);}

#ifdef _VUDEBUG
    {int vuxRet =  VuxIsDebugPlug() ? _14_CListTest_vpls_(pDatas) : _14_CListTest(pDatas);
#else
    {int vuxRet = _14_CListTest_vpls_(pDatas);
#endif
    TEST_ASSERT_RETURN("int", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_14_CListTest(clist_t*))
