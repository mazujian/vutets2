...
//VuxInter/Demo/_2T_SendMail.c/_05_read_mails_from_file_9bec7101.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/_05_read_mails_from_file(FILE*))
#ifdef _VUDEBUG
static Mail* _05_read_mails_from_file_vpls_(FILE* p_file){ VuxDbgBrkP()
{
    Mail* p_mails = 0;
    Mail* p_prev = 0;
    ...
    size_t buf_size = _VALUE_(size_t,buf_size,'a',0/*10240*/);
    size_t size;
    ...
    p_buf = (char*)vmck_malloc(buf_size);
    if(p_buf == 0)
    {
        return 0;
    }
    ...
    while(1)
    {
        ...
        if(size == buf_size)
        {
            if(p_buf[size-1] != '\n')
            {
                offset = _04_read_tail(p_buf, buf_size, &p_tail);  
        ...
        //解析缓冲区中的mail列表
        p_mail = _03_read_mails(p_buf, buf_size);
        if(p_mail != 0)
        {
            //添加到整体列表中
            if(p_prev == 0)
...
#else
static Mail* _05_read_mails_from_file_vpls_(FILE* p_file){VUX_VARIABLE(3)  __VUFNINFO__("_05_read_mails_from_file","Mail*",0);VuxDbp(0,0,38);
{
VUXRBV(1);VUXRLNFVB(1,1,1);    Mail* p_mails = 0; VUXLV("Mail*",p_mails,1);
VUXRLNFVB(2,2,2);    Mail* p_prev = 0; VUXLV("Mail*",p_prev,2);
    /*[学习提示：局部输入]	用途：解决测试难题，降低测试难度，更多请看_5T_InsideIO.cpp。    添加：在左边函数代码窗口，选择要替换的代码或点击输入位置，	        右键菜单选择"局部输出(...)"    填值：与一般数据一样。    举例：代码中，buf_size为10240，要准备超过10k的数据文件很麻烦，	        可用局部输入设为较小的数。这种情形下，至少要有一个用例		    使用原值，以保证原值得以测试。	*/
VUXRLNFVB(3,3,3);    size_t buf_size = _VALUE_(size_t,buf_size,'a',0/*10240*/); VUXLV("size_t",buf_size,3);
...
VUXRB(3);VUXRLNF(9,10);    p_buf = (char*)vmck_malloc(buf_size);
...
VUXRB(9);VUXRLNF(19,24);        offset = 0;
        ...
        {VUXRBR(12);{
            ...
            {VUXRBR(13);{
VUXRB(10);VUXRLNF(21,28);                offset = _04_read_tail(p_buf, buf_size, &p_tail);  
                //如果有被截断的mail，则offset>0且p_tail!=NULL
                ;VUXRLNF(22,29);
            }}
VUXRBR_ELSE(14);
...
VUXRBR_ELSE(15);
        ...
        //解析缓冲区中的mail列表
VUXRB(11);VUXRLNF(23,30);        p_mail = _03_read_mails(p_buf, buf_size);
...
VUXRB(12);VUXRLNF(24,33);                p_mails = p_mail;
            ...
            {VUXRBR(18);{
VUXRB(13);VUXRLNF(25,34);                _09_add_to_tail(p_prev, p_mail);
            }}
...
static Mail* _05_read_mails_from_file_vpls(FILE* p_file)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return _05_read_mails_from_file(p_file);}
	else if(VuxIsRepeatRun()) {return _05_read_mails_from_file_vpls_(p_file);}

#ifdef _VUDEBUG
    {Mail* vuxRet =  VuxIsDebugPlug() ? _05_read_mails_from_file_vpls_(p_file) : _05_read_mails_from_file(p_file);
#else
    {Mail* vuxRet = _05_read_mails_from_file_vpls_(p_file);
#endif
    TEST_ASSERT_RETURN("Mail*", &vuxRet);
    VuxResetRunTimes();
    return vuxRet;}
}
//$$VUNIT(END,TESTFUNCPLUG/_05_read_mails_from_file(FILE*))
