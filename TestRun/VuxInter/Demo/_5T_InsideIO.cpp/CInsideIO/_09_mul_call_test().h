...
//VuxInter/Demo/_5T_InsideIO.cpp/CInsideIO/_09_mul_call_test().h

//$$VUNIT(BEGIN,TESTFUNCPLUG/CInsideIO::_09_mul_call_test())
#ifdef _VUDEBUG
int CInsideIO::_09_mul_call_test_vpls_(){ VuxDbgBrkP()
{
...
#else
int CInsideIO::_09_mul_call_test_vpls_(){__VUFNINFO__("_09_mul_call_test","int",0);VuxDbp(0,0,13);
{
    /*[学习提示：底层输入之三]    多次调用同一底层函数（包括循环形成的多次调用），如需每次输出不同值，    可用分号隔开多个值。如值的数量不足，不足部分使用最后一个。	*/
...
int CInsideIO::_09_mul_call_test_vpls()
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return CInsideIO::_09_mul_call_test();}
...
//$$VUNIT(END,TESTFUNCPLUG/CInsideIO::_09_mul_call_test())
