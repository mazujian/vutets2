...
//VuxInter/Demo/_5T_InsideIO.cpp/CInsideIO/_08_call_sub_test_3921b90f.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/CInsideIO::_08_call_sub_test(int))
#ifdef _VUDEBUG
void CInsideIO::_08_call_sub_test_vpls_(int a){ VuxDbgBrkP()
{
...
#else
void CInsideIO::_08_call_sub_test_vpls_(int a){__VUFNID__(0);VuxDbp(0,0,4);
{
    /*[学习提示：底层输入之二]    底层函数崩溃或超时，可设skip it跳过。    很多时候，要测试在某种输入下是否调用某个函数，这时可以利用底层    输入的call times变量，在表格中设置它的预期调用次数。	*/
...
void CInsideIO::_08_call_sub_test_vpls(int a)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {CInsideIO::_08_call_sub_test(a); return;}
...
//$$VUNIT(END,TESTFUNCPLUG/CInsideIO::_08_call_sub_test(int))
