...
//VuxInter/Demo/_3T_MyClass.cpp/CMyClass/_02_Response_bfab2c1d.h

//$$VUNIT(BEGIN,TESTFUNCPLUG/CMyClass::_02_Response(stream_t))
#ifdef _VUDEBUG
BOOL CMyClass::_02_Response_vpls_(stream_t pMsg){ VuxDbgBrkP()
{
    //解释申请信息获取用户信息
    UserInfo info;
...
#else
BOOL CMyClass::_02_Response_vpls_(stream_t pMsg){VUX_VARIABLE(1)  __VUFNINFO__("_02_Response","BOOL",0);VuxDbp(0,0,22);
{
    //解释申请信息获取用户信息
...
VUXRBR_ELSE(7);
        ...
        //拼接短信
VUXRB(6);VUXRLNF(15,18);        msg += ',';
VUXRLNF(16,19);        msg += pJob->title;
    }}VUXRBR_OUT(1, 3);


    if((VUXRCS0(4,20)&&(!msg.IsEmpty())&&VUXRC1(4)))
    {VUXRBR(8);{VUXRB(7);
        _OUTPUT_(CString,msg,'B'); //最后一条短信
        /*[学习提示]        用局部输出判断临时的或未输出的计算结果。	    */
VUXRLNF(17,21);        SendMsg(msg, &info);
    }}
VUXRBR_ELSE(9);

VUXRB(8);VUXRLNF(18,22);    {BOOL __vurtv__= 1;VUXFRTY();}
}}
#endif

BOOL CMyClass::_02_Response_vpls(stream_t pMsg)
{
	VuxIncRunTimes();
    if(VuxRateTesting()) {return CMyClass::_02_Response(pMsg);}
...
//$$VUNIT(END,TESTFUNCPLUG/CMyClass::_02_Response(stream_t))
